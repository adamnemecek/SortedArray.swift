...
extension SortedArray : SetAlgebra {
    ...
    public mutating func formSymmetricDifference(_ other: SortedArray) {
    ...
    public mutating func formIntersection(_ other: SortedArray) {
    ...
    ///
    /// - Parameter other: A set of the same type as the current set.
    public mutating func formUnion(_ other: SortedArray) {
        self = union(other)
    }
    ...
    public func subtracting(_ other: SortedArray) -> SortedArray {
        var cp = self
        other.forEach {
            cp.remove($0)
        }
...
//    }
    ...
    /// - Parameter other: A set of the same type as the current set.
    /// - Returns: A new set.
    public func symmetricDifference(_ other: SortedArray) -> SortedArray {
        var cp = self
    ...
    public func intersection(_ other: SortedArray) -> SortedArray {
        var cp = self
        cp.content = cp.filter { !other.contains($0) }
        return cp
    }
    ...
    ///   distinguishable (e.g. via `===`), which of these elements is present
    ///   in the result is unspecified.
    public func union(_ other: SortedArray) -> SortedArray {
        return SortedArray(content + other.content)
    }
